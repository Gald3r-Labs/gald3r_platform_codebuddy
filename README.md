...
<h1 align="center">Gald3r for CodeBuddy (Tencent)</h1>

<p align="center">
  The <strong>CodeBuddy (Tencent) edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside CodeBuddy (Tencent)</strong>. Part of the Gald3r framework
...
This is the **CodeBuddy (Tencent) distribution** of Gald3r. The [`codebuddy/`](./codebuddy/) folder is a
complete, ready-to-deploy gald3r setup tuned for CodeBuddy (Tencent) -- drop its contents into your
...
cp -r gald3r_platform_codebuddy/codebuddy/. /path/to/your/project/
```

Then open the project in CodeBuddy (Tencent) and type `@g-status` to confirm gald3r loaded.

> The `codebuddy/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
